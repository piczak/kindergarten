 ...
 *
 * CheckboxTree na bazie DynamicTree (c) Michał Młodziński
 */
...
function CheckboxTree(id) {
    this.inputName = 'list[]';
    this.foldersAsCheckboxes = false;
    this.path = "images/";
    this.img = {
    ...
    this.parse = function(nodes, tree) {
        for (var i = 0; i < nodes.length; i++) {
            if (nodes[i].nodeType == 1) {
                ...
                if (nodes[i].dataset.id) {
                    nodes[i].id = this.id + "-" + nodes[i].dataset.id;
                }
                ...
                node.dataset    = nodes[i].dataset;
                node.inputName  = this.inputName;
    ...
    function Node(id, text, parentNode, childNodes, isDoc, isFolder, inputName) {
        this.id = id;
        this.text = text;
        ...
        this.isLast = function() {
            ...
            throw "CheckboxTree.Node.isLast() failed, this func cannot be called for the root element";
        };
        this.toHtml = function() {
            var s = '<div class="?" id="?" data-id="?" data-checked="?">'.format(this.className ? this.className : (this.isFolder ? "folder" : "doc"), this.id, this.dataset.id, this.dataset.checked);
            if (this.isFolder) {
                var nodeIcon;
                ...
                if (self.foldersAsCheckboxes) {
                    s += '<input type="?" name="?" value="?" ? /> '.format((this.dataset.type == 'checkbox' ? 'checkbox' : 'radio'), this.inputName, this.dataset.id, (this.dataset.checked  == 'true' ? 'checked="true"' : ''));
                } else {
            ...
            if (this.isDoc) {
                s += '<img src="?" width="18" height="18" alt="" /><input type="?" name="?" value="?" ? />'.format((this.isLast() ? self.img.leafEnd : self.img.leaf), (this.dataset.type == 'checkbox' ? 'checkbox' : 'radio'), this.inputName, this.dataset.id, (this.dataset.checked  == 'true' ? 'checked="true"' : ''));
                s += '&nbsp;&nbsp;<a id="?-link" onclick="?.setActive(\'?\');" href="?"??>?</a>'.format(this.id, self.id, this.id, (this.href ? this.href : '#'), (this.title ? ' title="?"'.format(this.title) : ""), (this.target ? ' target="?"'.format(this.target) : ""), this.text);
            }
            s += '</div>';
            return s;
        };
